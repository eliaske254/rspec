...
RSpec.describe BiddingEngine do
  describe ".bid!" do
    let(:seller) { User.create(:email => 'jane@dawn.com', :password => 'mypassword') }
    let(:bidder) { User.create(:email => 'john@doe.com', :password => 'mypassword') }
    ...
    it "creates a new bid on an auction if bid is bigger than last bid on auction" do
